...
class MangaTranslator:
    ...
    def __init__(self, params: dict = None):
        ...
        # 会话文件夹：延迟创建，只在实际需要保存结果时才创建
        # Session folder: lazy creation, only created when actually saving results
        self.__session_folder = None  # 使用双下划线表示私有，通过属性访问
    ...
    def _session_folder(self) -> str:
        ...
        if self.__session_folder is None:
            self.__session_folder = self._create_session_folder()
            # 首次创建文件夹时也初始化日志
    ...
    @_session_folder.setter
    def _session_folder(self, value):
        """允许设置会话文件夹"""
        self.__session_folder = value
    
    def _create_session_folder(self) -> str:
        """创建会话文件夹，以启动时间命名"""
        from datetime import datetime
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        session_folder = os.path.join(BASE_PATH, 'result', timestamp)
        os.makedirs(session_folder, exist_ok=True)
        logger.info(f"Created session folder: {session_folder}")
        return session_folder
    
    def set_current_image_filename(self, filename: str):
        """设置当前处理图片的原始文件名"""
        self._current_image_filename = filename

    def set_batch_output_folder(self, batch_name: str):
        """设置批次输出文件夹名称"""
        self._batch_output_folder = batch_name

    async def translate_batch_with_context(self, images: List[Image.Image], config: 'Config', batch_name: str, progress_callback=None) -> List['Context']:
        """
        批次上下文翻译：同一批次的图片一起OCR，合并文本后批量翻译，保持上下文关联
        
        流程：
        1. 对所有图片进行预处理（检测、OCR、文本合并）
        2. 收集所有文本，一起发送给翻译API
        3. 将翻译结果分配回各个图片
        4. 分别渲染每张图片
        5. 结果保存到 result/{session}/{batch_name}/ 文件夹
        
        Args:
            images: 图片列表（按顺序）
            config: 翻译配置
            batch_name: 批次名称，用于输出文件夹命名
        ...
        # 创建批次输出文件夹
        batch_folder = os.path.join(self._session_folder, batch_name)
        os.makedirs(batch_folder, exist_ok=True)
        logger.info(f"Created batch output folder: {batch_folder}")
        
        await report_progress(f"开始预处理 {len(images)} 张图片...")
        
        # 阶段1：预处理所有图片（检测、OCR、文本合并）
        pre_contexts = []
        all_texts = []  # 收集所有文本，格式: [(page_idx, region_idx, text), ...]
        
        for i, image in enumerate(images):
            await report_progress(f"OCR处理中: {i+1}/{len(images)}")
            logger.info(f"Processing image {i+1}/{len(images)}")
            
            try:
                # 设置图片上下文
                self._set_image_context(config, image)
                logger.info(f"Image context set for image {i+1}")
                
                # 执行预处理（到翻译之前）
                print(f"[OCR] Starting _translate_until_translation for image {i+1}")
                logger.info(f"Starting _translate_until_translation for image {i+1}")
                ctx = await self._translate_until_translation(image, config)
                ctx.verbose = self.verbose
                print(f"[OCR] _translate_until_translation completed for image {i+1}")
                print(f"[OCR] Image {i+1}: textlines={len(ctx.textlines) if hasattr(ctx, 'textlines') and ctx.textlines else 0}")
                print(f"[OCR] Image {i+1}: text_regions={len(ctx.text_regions) if ctx.text_regions else 0}")
                logger.info(f"_translate_until_translation completed for image {i+1}")
                
                # 收集文本
                if ctx.text_regions:
                    for region_idx, region in enumerate(ctx.text_regions):
                        all_texts.append((i, region_idx, region.text))
                        print(f"[OCR] Image {i+1}, Region {region_idx}: '{region.text[:50]}...' " if len(region.text) > 50 else f"[OCR] Image {i+1}, Region {region_idx}: '{region.text}'")
                    logger.info(f"Image {i+1}: found {len(ctx.text_regions)} text regions")
                else:
                    print(f"[OCR] Image {i+1}: no text regions found!")
                    logger.info(f"Image {i+1}: no text regions found")
                
                pre_contexts.append((ctx, config))
                logger.debug(f"Image {i+1} pre-processing completed, found {len(ctx.text_regions) if ctx.text_regions else 0} text regions")
        ...
        print(f"[Translation] Pre-processing completed. Total texts collected: {len(all_texts)}")
        logger.info(f"Pre-processing completed. Total texts collected: {len(all_texts)}")
        
        # 清理输入图片列表，释放内存
        images.clear() if hasattr(images, 'clear') else None
        
        if not all_texts:
            await report_progress("未检测到文本，跳过翻译")
            print("[Translation] No texts detected, skipping translation")
            logger.warning("No texts detected, skipping translation")
            # 直接返回原图
            results = []
            for ctx, cfg in pre_contexts:
                ctx.result = ctx.input
                results.append(ctx)
            return results
        
        # 阶段2：批量翻译所有文本
        print(f"[Translation] Starting batch translation phase with {len(all_texts)} texts")
        logger.info(f"Starting batch translation phase with {len(all_texts)} texts")
        await report_progress(f"批量翻译中: 共 {len(all_texts)} 条文本...")
        ...
        marked_texts = []
        current_page = -1
        for page_idx, region_idx, text in all_texts:
        ...
        try:
            # 使用第一个context进行翻译
            first_ctx = pre_contexts[0][0]
            print(f"[Translation] Calling _batch_translate_texts with {len(marked_texts)} texts")
            print(f"[Translation] Texts to translate: {marked_texts[:3]}...")  # 只打印前3个
            logger.info(f"Calling _batch_translate_texts with {len(marked_texts)} texts")
            logger.info(f"Texts to translate: {marked_texts[:3]}...")  # 只打印前3个
            translations = await self._batch_translate_texts(marked_texts, config, first_ctx)
            print(f"[Translation] Translation completed, got {len(translations)} translations")
            print(f"[Translation] Sample translations: {translations[:3]}...")  # 打印前3个翻译结果
            logger.info(f"Translation completed, got {len(translations)} translations")
            
            # 将翻译结果分配回各个region
            for idx, (page_idx, region_idx, _) in enumerate(all_texts):
                if idx < len(translations):
                    ctx = pre_contexts[page_idx][0]
                    if ctx.text_regions and region_idx < len(ctx.text_regions):
                        ctx.text_regions[region_idx].translation = translations[idx]
                        ctx.text_regions[region_idx].target_lang = config.translator.target_lang
                        ctx.text_regions[region_idx]._alignment = config.render.alignment
                        ctx.text_regions[region_idx]._direction = config.render.direction
            
            await report_progress("翻译完成")
            logger.info("Translation phase completed successfully")
            
        except Exception as e:
            import traceback
            logger.error(f"Batch translation error: {e}")
            logger.error(f"Traceback: {traceback.format_exc()}")
            await report_progress(f"翻译错误: {str(e)}")
        ...
        # 清理 pre_contexts 中的 OCR 中间数据（保留必要的 text_regions 和 mask）
        import gc
        for ctx, cfg in pre_contexts:
            # 清理 OCR 阶段的大对象，但保留渲染需要的数据
            if hasattr(ctx, 'textlines'):
                ctx.textlines = None
            if hasattr(ctx, 'img_colorized'):
                ctx.img_colorized = None
            if hasattr(ctx, 'upscaled'):
                ctx.upscaled = None
        
        gc.collect()
        
        # 阶段3：渲染所有图片（mask refinement, inpainting, rendering）
        logger.info(f"Starting rendering phase for {len(pre_contexts)} images")
        await report_progress("开始渲染...")
        results = []
        for i, (ctx, cfg) in enumerate(pre_contexts):
            await report_progress(f"渲染中: {i+1}/{len(pre_contexts)}")
            print(f"[Render] Image {i+1}: text_regions={len(ctx.text_regions) if ctx.text_regions else 0}")
            
            # 每处理3张图片触发一次垃圾回收和GPU清理，更积极地释放内存
            if i > 0 and i % 3 == 0:
                gc.collect()
                try:
                    import torch
            ...
            try:
                if ctx.text_regions:
                    print(f"[Render] Image {i+1}: has text_regions, starting rendering...")
                    # Mask refinement
                    if ctx.mask is None:
                        try:
                            ctx.mask = await self._run_mask_refinement(cfg, ctx)
                        except Exception as e:
                    ...
                    # Inpainting
                    try:
                        ctx.img_inpainted = await self._run_inpainting(cfg, ctx)
                    except Exception as e:
                    ...
                    # Rendering
                    try:
                        ctx.img_rendered = await self._run_text_rendering(cfg, ctx)
                    except Exception as e:
                    ...
                    # 处理缩放
                    if cfg.upscale.revert_upscaling and ctx.result:
                        ctx.result = ctx.result.resize(ctx.input.size)
                else:
                    print(f"[Render] Image {i+1}: no text_regions, using original image")
                    ctx.result = ctx.input
                ...
                if ctx.result:
                    # 确保目录存在
                    os.makedirs(batch_folder, exist_ok=True)
                    output_path = os.path.join(batch_folder, f"{i+1:03d}_final.png")
                    # 直接使用 PIL 保存（更可靠，支持 RGBA）
                    ctx.result.save(output_path, format='PNG')
                    print(f"[Render] Image {i+1}: saved to {output_path}")
                    logger.info(f"Saved batch result: {output_path}")
                
                # 清理中间数据，只保留 result（减少内存占用）
                ctx.cleanup_large_objects()
                # 保留 result 用于返回
                
                results.append(ctx)
                
            except Exception as e:
                logger.error(f"Image {i+1} rendering error: {e}")
                ctx.result = ctx.input
                results.append(ctx)
        
        await report_progress(f"批次 {batch_name} 完成，共处理 {len(results)} 张图片")
        ...
        try:
            ...
            log_memory_usage(f"[Batch {batch_name}] After cleanup: ")
        except Exception:
            pass
    ...
    def _cleanup_batch_data(self):
        ...
        try:
            from .utils.memory_monitor import log_memory_usage
            log_memory_usage("[Cleanup] Before: ")
        except Exception:
            pass
        
        # 清理翻译历史（保留最近的上下文用于连续翻译）
        history_before = len(self.all_page_translations)
        if self.context_size > 0 and len(self.all_page_translations) > self.context_size:
            self.all_page_translations = self.all_page_translations[-self.context_size:]
            self._original_page_texts = self._original_page_texts[-self.context_size:]
        ...
        if history_before > 0:
            logger.info(f"Cleaned translation history: {history_before} -> {len(self.all_page_translations)}")
        
        # 清理批量处理的上下文
        batch_ctx_count = len(self._batch_contexts)
        self._batch_contexts.clear()
        self._batch_configs.clear()
        if batch_ctx_count > 0:
            logger.info(f"Cleaned batch contexts: {batch_ctx_count} items")
        
        # 清理图片上下文缓存
        img_ctx_count = len(self._saved_image_contexts)
        self._saved_image_contexts.clear()
        self._current_image_context = None
        if img_ctx_count > 0:
            logger.info(f"Cleaned image contexts: {img_ctx_count} items")
        
        # 清理翻译器缓存中的历史数据
        try:
            from .translators import translator_cache
            for key, translator in list(translator_cache.items()):
                # 清理翻译器内部可能的缓存
                if hasattr(translator, 'token_count'):
                    translator.token_count = 0
                if hasattr(translator, 'token_count_last'):
                    translator.token_count_last = 0
                # 清理可能的上下文缓存
                if hasattr(translator, '_prev_context'):
                    translator._prev_context = None
                if hasattr(translator, 'langSamples'):
                    translator.langSamples = None
                if hasattr(translator, '_json_sample'):
                    translator._json_sample = None
        except Exception as e:
            logger.debug(f"Failed to cleanup translator cache: {e}")
        
        # 注意：不清理 ocr_cache 和 detector_cache，因为重新加载模型开销很大
        # 这些模型会在 models_ttl 超时后自动卸载
        ...
        try:
            from .utils.memory_monitor import log_memory_usage
            log_memory_usage("[Cleanup] After: ")
        except Exception:
            pass
        
        logger.info("Batch data cleanup completed")
    ...
    def _setup_log_file(self):
        ...
        try:
            ...
            # 注意：不再重定向 print 和 Rich Console
            # 这样可以减少每次输出的内存和 I/O 开销
            # 如果需要记录详细日志，请使用 logger.info() 而不是 print()
    ...
    def parse_init_params(self, params: dict):
        ...
        kernel_size_val = params.get('kernel_size', 3)
        self.kernel_size = int(kernel_size_val) if kernel_size_val is not None else 3
        # Set input files
    ...
    async def translate(self, image: Image.Image, config: Config, image_name: str = None, skip_context_save: bool = False) -> Context:
        ...
        if not skip_context_save and ctx.text_regions:
            ...
            if self.context_size > 0 and len(self.all_page_translations) > self.context_size:
                ...
                # 不需要上下文时，完全清理（保留最后一个用于当前翻译）
                if len(self.all_page_translations) > 1:
                    self.all_page_translations = self.all_page_translations[-1:]
                    self._original_page_texts = self._original_page_texts[-1:]
    ...
    async def _translate(self, config: Config, ctx: Context) -> Context:
        ...
        result_ctx = await self._revert_upscale(config, ctx)
        
        # 清理中间数据，释放内存（保留 result 和必要的调试信息）
        ctx.cleanup_large_objects()
        ...
        # 记录清理后的内存使用情况（每10张图片记录一次）
        try:
            if not hasattr(self, '_translate_count'):
                self._translate_count = 0
            self._translate_count += 1
            if self._translate_count % 10 == 0:
                from .utils.memory_monitor import log_memory_usage
                log_memory_usage(f"[Single] After {self._translate_count} images: ")
        except Exception:
            pass
    ...
    async def _revert_upscale(self, config: Config, ctx: Context):
        ...
        # 始终保存 final.png 到 result 文件夹
        # Always save final.png to result folder for all modes (web frontend, CLI, etc.)
        if ctx.result:
            try:
                ...
                if success:
                    logger.info(f"Saved translation result to: {final_path}")
                else:
                    logger.warning(f"Failed to save result image: {final_path}")
            except Exception as e:
                logger.error(f"Error saving final.png: {e}")
                logger.debug(f"Exception details: {traceback.format_exc()}")
    ...
    async def _run_text_rendering(self, config: Config, ctx: Context):
        ...
        # manga2eng currently only supports horizontal left to right rendering
            ...
            logger.info(f"Render config: font_size={config.render.font_size}, offset={config.render.font_size_offset}, min={config.render.font_size_minimum}, max={config.render.font_size_maximum}, compression={config.render.font_size_compression}")
            output = await dispatch_rendering(ctx.img_inpainted, ctx.text_regions, self.font_path, config.render.font_size,
                                              ...
                                              config.render.font_size_maximum,
                                              config.render.font_size_compression,
                                              not config.render.no_hyphenation, ctx.render_mask, config.render.line_spacing)
        return output

    def _result_path(self, path: str) -> str:
        """
        Returns path to result folder where intermediate images are saved.
        
        For final.png: Uses session folder + original filename
        For debug images (verbose mode): Uses session folder + image subfolder
        """
        # 如果是保存最终结果（final.png），使用原始文件名
        if path == 'final.png' and self._current_image_filename:
            # 使用会话文件夹 + 原始文件名
            result_path = os.path.join(self._session_folder, self._current_image_filename)
            # 确保目录存在
            os.makedirs(os.path.dirname(result_path) if os.path.dirname(result_path) else self._session_folder, exist_ok=True)
            return result_path
        
        # verbose模式下的调试图片使用子文件夹
        if self.verbose:
            image_subfolder = self._get_image_subfolder()
            if image_subfolder:
                if self.result_sub_folder:
                    ...
                    result_path = os.path.join(self._session_folder, 'debug', image_subfolder, path)
                # 确保目录存在
        ...
        if self.result_sub_folder:
            ...
            result_path = os.path.join(self._session_folder, path)
    ...
    async def _translate_until_translation(self, image: Image.Image, config: Config) -> Context:
        ...
        if not ctx.textlines:
            await self._report_progress('skip-no-regions', True)
            ctx.result = ctx.upscaled
            ctx.text_regions = []  # 确保text_regions被初始化
            return ctx  # 直接返回，不调用_revert_upscale（批次模式下会单独处理）
