...
class CustomOpenAiTranslator(ConfigGPT, CommonTranslator):
    # 设为 0 禁用无效翻译重试，避免 Claude 等模型因格式问题导致多次 API 调用
    # 如果需要重试，可以在 gpt_config.yaml 中设置 invalid_repeat_count
    _INVALID_REPEAT_COUNT = 0
    _MAX_REQUESTS_PER_MINUTE = 40  # 每分钟最大请求次数
    _TIMEOUT = 60  # 在重试之前等待服务器响应的时间（秒）
    ...
    async def _translate(self, from_lang: str, to_lang: str, queries: List[str]) -> List[str]:
        ...
        # 日志：开始翻译
        print(f"[shared] Running translation")
        print(f"[CustomOpenAI] Model: {self.model or CUSTOM_OPENAI_MODEL}")
        print(f"[CustomOpenAI] Translating {len(queries)} texts from {from_lang} to {to_lang}")
        
        prompt_count = 0
        total_prompts = sum(1 for _ in self._assemble_prompts(from_lang, to_lang, queries.copy()))

        for prompt, query_size in self._assemble_prompts(from_lang, to_lang, queries):
            prompt_count += 1
            print(f"[CustomOpenAI] Sending request {prompt_count}/{total_prompts} ({query_size} texts)")
            ...
            while True:
                ...
                try:
                    ...
                    print(f"[CustomOpenAI] Request completed in {request_duration:.2f}s")
                    break
