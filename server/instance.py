...
class ExecutorInstance(BaseModel):
    ...
    async def sent_stream(self, image: Image, config: Config, sender: NotifyType, filename: str = None):
        await fetch_data_stream("http://"+self.ip+":"+str(self.port)+"/execute/translate", image, config, sender, headers=self._nonce_headers(), filename=filename)
