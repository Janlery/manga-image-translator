...
class QueueElement:
    ...
    def __init__(self, req: Request, image: Image.Image, config: Config, length, filename: str = None):
        self.req = req
        if length > 10:
            ...
            self.image = image
        self.config = config
        self.filename = filename or "unknown.png"
        ...
        if queue_pos < executor_instances.free_executors():
            ...
            try:
                ...
                if isinstance(task, BatchQueueElement):
                    ...
                    if notify:
                        await instance.sent_stream(task.image, task.config, notify, filename=task.filename)
                    else:
